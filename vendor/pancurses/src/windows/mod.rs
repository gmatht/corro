...
// Win95/9x compatibility (vendored patch): for rust9x targets, the win32
// module below is selected via its target-aware cfg (see next block). The
// win32a module assumes the GDI flavor (draws via ExtTextOutW — a no-op
// stub on Windows 95) and pulls in winreg font tweaks.
...
#[cfg(any(feature = "win32", all(target_family = "rust9x", target_env = "msvc")))]
mod win32;
#[cfg(any(feature = "win32", all(target_family = "rust9x", target_env = "msvc")))]
