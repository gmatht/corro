...
bool PDC_check_key(void)
{
    ...
    /* Windows 9x: PeekConsoleInput on a console input handle always reports
       an empty queue (broken there), while GetNumberOfConsoleInputEvents and
...
#ifdef _WIN32_W9X_CONSOLE
    /* Windows 9x: SetConsoleMode with QuickEdit flags (0x0080 is an NT-only
       extended flag) leaves the input handle in a state where key events are
       never delivered to GetNumberOfConsoleInputEvents.  Use exactly the
       mode empirically verified to work on 9x (ENABLE_WINDOW_INPUT |
       ENABLE_MOUSE_INPUT). */
    if (_pdc_is_win9x())
        SetConsoleMode(pdc_con_in, ENABLE_WINDOW_INPUT | ENABLE_MOUSE_INPUT);
    else
