...
int PDC_scr_open(int argc, char **argv)
{
...
#ifdef _WIN32_W9X_CONSOLE
    /* Host apps may redirect the standard handles (e.g. corro redirects
       stdout/stderr to a log file on Win9x); PDCurses must draw on the
       REAL console, not the redirected file. Open CONOUT$/CONIN$ directly
       (present on Windows 95 as well). */
    {
        HANDLE con_out = CreateFileA("CONOUT$",
            GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ | FILE_SHARE_WRITE,
            NULL, OPEN_EXISTING, 0, NULL);
        if (con_out != INVALID_HANDLE_VALUE)
            std_con_out = pdc_con_out = con_out;
        HANDLE con_in = CreateFileA("CONIN$",
            GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ | FILE_SHARE_WRITE,
            NULL, OPEN_EXISTING, 0, NULL);
    ...
    /* Windows 95's GetConsoleScreenBufferInfo returns garbage (srWindow
       uninitialized, dwSize.Y and dwMaximumWindowSize bogus); normalize
       both structures from the DOS box default so the LINES/COLS checks
       and window-restore logic operate on sane values. */
    if (_pdc_is_win9x())
    {
        csbi.dwSize.X = 80;
        csbi.dwSize.Y = 25;
    ...
    if (is_nt &&
        ((str = getenv("PDC_RESTORE_SCREEN")) == NULL || *str != '0'))
    {
        /* Create a new console buffer. Windows 95's
           SetConsoleActiveScreenBuffer doesn't actually switch the visible
           buffer, so keep drawing into the original (visible) buffer there. */
