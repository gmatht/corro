...
//
// Win95 note: upstream implements these with the Win7+ IFileDialog COM API
// (and SHCreateItemFromParsingName). That import does not exist on Windows 95
// and, being static, makes the whole executable refuse to load. The dialog
// now lives entirely in `resources/file_dialog.rs` on top of the Win95-era
// comdlg32 ANSI common dialogs (GetOpenFileNameA / GetSaveFileNameA) and the
// shell folder browser (SHBrowseForFolderA), so there is no COM helper here
// any more.
