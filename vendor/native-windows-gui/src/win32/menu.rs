...
/**
    Configure the menu to report item activations as `WM_MENUCOMMAND` instead
    of `WM_COMMAND`.

    Win95 note: `SetMenuInfo`/`MNS_NOTIFYBYPOS` are **Win98+** USER32 exports,
    so this is a no-op there — the import would also make the whole executable
    refuse to load on Windows 95. On Win9x menus report activations through the
    classic `WM_COMMAND` path instead (see `window.rs`: menu item ids are all
    >= `CUSTOM_ID_BEGIN`, so the `_` arm of the `WM_COMMAND` class-name match
    routes them to `Event::OnMenuItemSelected`). The public behaviour is
    identical: the caller gets one `OnMenuItemSelected` per activation.
*/
unsafe fn use_menu_command(h: HMENU) {
    // Only NT5/Win98 and later have SetMenuInfo. Importing it unconditionally
    // would break the Win95 loader, so it is resolved at runtime and simply
    // skipped when absent.
    use winapi::shared::minwindef::DWORD;
    use winapi::um::libloaderapi::{GetModuleHandleA, GetProcAddress};

    type SetMenuInfoFn = unsafe extern "system" fn(HMENU, *mut MenuInfoOpaque) -> i32;

    #[repr(C)]
    struct MenuInfoOpaque {
        cb_size: DWORD,
        f_mask: DWORD,
        dw_style: DWORD,
        cy_max: DWORD,
        hbr_back: *mut winapi::ctypes::c_void,
        dw_context_help_id: DWORD,
        dw_menu_data: usize,
    }

    let user32 = GetModuleHandleA(b"user32.dll\0".as_ptr() as *const _);
    if user32.is_null() {
        return;
    }
    let proc = GetProcAddress(user32, b"SetMenuInfo\0".as_ptr() as *const _);
    if proc.is_null() {
        return; // Win95/Win98-Gold: menus already report WM_COMMAND
    }
    let set_menu_info: SetMenuInfoFn = mem::transmute(proc);

    // MIM_STYLE = 0x10, MNS_NOTIFYBYPOS = 0x08
    const MIM_STYLE: DWORD = 0x0000_0010;
    const MNS_NOTIFYBYPOS: DWORD = 0x0000_0008;

    let mut info = MenuInfoOpaque {
        cb_size: mem::size_of::<MenuInfoOpaque>() as DWORD,
        f_mask: MIM_STYLE,
        dw_style: MNS_NOTIFYBYPOS,
        cy_max: 0,
        hbr_back: ptr::null_mut(),
        dw_context_help_id: 0,
        dw_menu_data: 0,
    };

    set_menu_info(h, &mut info);
}
