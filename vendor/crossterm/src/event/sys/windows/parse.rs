...
use winapi::um::{
    ...
    winuser::{
        GetForegroundWindow, GetKeyboardLayout, GetWindowThreadProcessId, VK_BACK,
...
fn get_char_for_key(key_event: &KeyEventRecord) -> Option<char> {
    ...
    let ret = unsafe {
        ...
        // USER32.DLL:ToUnicodeEx". Resolve dynamically, falling back to
        // ToAsciiEx (present since Win3.1/95) with its ANSI result widened.
        win95_to_unicode_ex(
            virtual_key_code,
            virtual_scan_code,
            key_state.as_ptr(),
            ...
            active_keyboard_layout as *mut std::os::raw::c_void,
        )
    };
...
// --- Win95 compatibility (vendored patch) -----------------------------------
// USER32 on Windows 95 has no ToUnicodeEx (Win2000+ API). Resolve it
// dynamically so it stays out of the import table; fall back to ToAsciiEx
// (present since Win3.1/95) and widen its ANSI result to UTF-16. The hkl
// argument is ignored by the fallback (ToAsciiEx uses the current thread's
// layout, which is also what ToUnicodeEx does when the layout lookup above
// fails). Only exact single-char results map; dead keys return 0 like
// ToUnicodeEx's "no character" case.
#[allow(clippy::too_many_arguments, non_snake_case)]
fn win95_to_unicode_ex(
    vk: u32,
    scan: u32,
    key_state: *const u8,
    utf16_buf: *mut u16,
    buf_len: i32,
    flags: u32,
    hkl: *mut std::os::raw::c_void,
) -> i32 {
    unsafe extern "system" {
        fn GetModuleHandleA(name: *const u8) -> *mut std::os::raw::c_void;
        fn LoadLibraryA(name: *const u8) -> *mut std::os::raw::c_void;
        fn GetProcAddress(module: *mut std::os::raw::c_void, name: *const u8) -> *const std::os::raw::c_void;
    }
    unsafe {
        let mut user32 = GetModuleHandleA(b"user32\0".as_ptr());
        if user32.is_null() {
            user32 = LoadLibraryA(b"user32\0".as_ptr());
        }
        if user32.is_null() || buf_len < 1 {
            return 0;
        }
        type ToUnicodeExFn =
            unsafe extern "system" fn(u32, u32, *const u8, *mut u16, i32, u32, *mut std::os::raw::c_void) -> i32;
        let p = GetProcAddress(user32, b"ToUnicodeEx\0".as_ptr());
        if !p.is_null() {
            let f: ToUnicodeExFn = std::mem::transmute(p);
            return f(vk, scan, key_state, utf16_buf, buf_len, flags, hkl);
        }
        type ToAsciiExFn =
            unsafe extern "system" fn(u32, u32, *const u8, *mut u16, u32, *mut std::os::raw::c_void) -> i32;
        let p = GetProcAddress(user32, b"ToAsciiEx\0".as_ptr());
        if p.is_null() {
            return 0;
        }
        let f: ToAsciiExFn = std::mem::transmute(p);
        let mut ansi: [u16; 2] = [0; 2];
        let n = f(vk, scan, key_state, ansi.as_mut_ptr(), flags, hkl);
        if n < 1 {
            return 0; // dead key / no translation
        }
        // ANSI code page byte; US/international layouts are ASCII here.
        *utf16_buf = (ansi[0] & 0xff) as u16;
        1
    }
}
