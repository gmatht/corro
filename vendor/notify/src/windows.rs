...
use windows_sys::Win32::Foundation::{
    CloseHandle, ERROR_OPERATION_ABORTED, BOOL, HANDLE, INVALID_HANDLE_VALUE, WAIT_OBJECT_0,
};
...
use windows_sys::Win32::System::IO::{OVERLAPPED, LPOVERLAPPED_COMPLETION_ROUTINE};

const BUF_SIZE: u32 = 16384;

// --- Win95 compatibility (vendored patch) -----------------------------------
// Windows 95's KERNEL32 has neither ReadDirectoryChangesW nor CancelIo (both
// arrived in Win98/NT4). Statically importing them would make the Win95
// loader reject the whole executable with "linked to missing export
// KERNEL32.DLL:...", so both are resolved dynamically here. When the API is
// unavailable (Win95), the watcher degrades gracefully: start_read's error
// path (ret == 0) releases the semaphore and surfaces the error through the
// channel, and CancelIo becomes a no-op (no pending overlapped IO exists to
// cancel on such systems).
unsafe fn win95_dynamic_kernel32_proc(name: &[u8]) -> isize {
    unsafe extern "system" {
        fn GetModuleHandleA(name: *const u8) -> HANDLE;
        fn GetProcAddress(module: HANDLE, name: *const u8) -> isize; // FARPROC
    }
    unsafe {
        ...
        GetProcAddress(k32, name.as_ptr())
    }
}

unsafe fn win95_cancel_io(handle: HANDLE) -> i32 {
    unsafe {
        let p = win95_dynamic_kernel32_proc(b"CancelIo\0");
        if p == 0 {
            return 0;
        }
        let f: unsafe extern "system" fn(HANDLE) -> i32 = std::mem::transmute(p);
        f(handle)
    }
}

#[allow(clippy::too_many_arguments)]
unsafe fn win95_read_directory_changes_w(
    handle: HANDLE,
    buffer: *mut c_void,
    buf_size: u32,
    subdir: BOOL,
    ...
    unsafe {
        let p = win95_dynamic_kernel32_proc(b"ReadDirectoryChangesW\0");
        if p == 0 {
            return 0;
        }
        ...
        f(handle, buffer, buf_size, subdir, flags, bytes, overlapped, completion)
    }
}
...
fn stop_watch(ws: &WatchState, meta_tx: &Sender<MetaEvent>) {
    unsafe {
        let cio = win95_cancel_io(ws.dir_handle);
...
fn start_read(rd: &ReadData, event_handler: Arc<Mutex<dyn EventHandler>>, handle: HANDLE) {
    ...
    unsafe {
        ...
        let ret = win95_read_directory_changes_w(
            handle,
