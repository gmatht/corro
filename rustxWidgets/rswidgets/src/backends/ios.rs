//! iOS backend core: UIKit object lifetime + callback registry.
//!
//! The Android sibling (`backends/android.rs`) keeps a `JNIEnv`/`jobject`
//! world alive with `GlobalRef`s and reaches the VM through `jni`. iOS has
//! no VM: the host app (`ios/corro`) hands us a live `UIView*` subtree from
//! Swift/ObjC, and we call UIKit through the Objective-C runtime — the same
//! "raw handle + registry" shape, no runtime to attach to.
//!
//! Three things live here, mirroring the Android module one-for-one:
//!
...
//!   UIKit keeps its hierarchy alive, but callbacks can hold handles after a
//!   view is removed, so owning a retain is what makes a dangling pointer
//!   impossible. Releasing on drop is deliberately *not* done: widgets are
//!   `Copy`-like handles shared across closures (same decision as Android's
//!   leaked `GlobalRef` list).
//! * **Callback registry** — Rust closures are keyed by a `u64` id; the
//!   ObjC side calls back into `#[no_mangle] extern "C"` entry points in the
//!   host cdylib, which dispatch by id here. This is the `RustCallback.java`
//!   trampoline from Android, minus the JVM.
...
//! shared GUI code compiles and runs unchanged on a host without UIKit.
...
mod ios_backend {
    ...
    // strings in UIKit's ABI (stable since the first iPhone OS), and this
    // keeps the dependency list at zero — which matters for the armv7s /
    // iOS 7 build path in `docs/IOS_GUIDELINES.md` §9 (an old SDK plus old
    // Rust toolchain plus a heavy ObjC binding stack is exactly where the
    // 32-bit build gets fragile).
    //
    // `objc_msgSend` is variadic and *must* be cast to the exact function
    // signature for each call (the ABI differs per return type: `CGFloat`
    // floats come back in different registers than integers on armv7).
    // Hence one typed `msg_send_*` helper per shape used here.
    ...
    /// Look up a class by name (`UIView`, `UILabel`, ...). Returns null when
    ...
    /// what UIKit itself does on first use.
    ...
    /// it to their own exact signature (the adapter's host-shim calls do
    ...
    /// `sel_registerName` for a selector name. Public so the adapter can pass
    ...
    /// every iOS target: 32-bit armv7 and 64-bit arm64 alike).
    ...
    ///
    /// `stringWithUTF8String:` is available from iOS 2.0, so this is safe on
    /// the iOS 7 path.
    ...
    /// The host's root view (the `ViewController`'s view). Set by
    /// [`init_with_root`]; all widget creation hangs off it.
    static ROOT_VIEW: Mutex<Option<usize>> = Mutex::new(None);
    /// The host's view controller, for presenting dialogs.
    static VIEW_CONTROLLER: Mutex<Option<usize>> = Mutex::new(None);

    /// Retains applied by this module, kept for the process lifetime. UIKit
    /// already owns its hierarchy; this list exists so a handle captured in a
    /// Rust closure stays valid even after the view leaves the hierarchy
    /// (removeFromSuperview does not deallocate while we hold a retain).
    ...
    /// Hand the backend the host's root view (and its view controller).
    ///
    /// Mirrors Android's `init_with_layout(env, activity, layout)`: after
    /// this, `create_*` builds real UIKit objects as subviews of `root`.
    /// Idempotent-safe in the sense that a second call replaces the stored
    /// root (matching the single-scene iOS model, where a second call would
    ...
    pub unsafe fn init_with_root(
        ...
        if root.is_null() {
            return Err("ios backend init: null root view".into());
        }
        ...
        *VIEW_CONTROLLER.lock().unwrap() = if vc.is_null() { None } else { Some(vc as usize) };
        ...
        log_ios("ios backend initialised");
        Ok(())
    }
    ...
    /// The host's view controller (for `presentViewController:`), if given.
    ...
    /// Write a debug line to the iOS system log under the `rswidgets` tag.
    /// Best-effort: silently dropped when the backend is not initialised.
    ///
    /// `NSLog` exists from iOS 2.0 and writes to the device console, which is
    /// what `xcrun simctl spawn booted log stream` and Console.app read — the
    /// equivalent of Android's `logcat -s rswidgets`.
    pub fn log_ios(msg: &str) {
        if !is_initialized() {
            return;
        }
        // NSLog (Foundation, a plain C function — not a message send)
        // writes to the device console: `xcrun simctl spawn booted log
        // stream` and Console.app read it, the equivalent of Android's
        // `logcat -s rswidgets`.
        //
    ...
    /// the shim calls `corro_ios_callback(id)` which lands in
    /// [`dispatch_callback`].
    ...
    pub fn dispatch_callback(id: u64) {
        ...
        if let Some(raw) = raw {
            ...
            // dispatch is single-threaded on the UIKit main thread.
            unsafe { (*raw)() };
        }
    }
    ...
    pub struct WidgetMeta {
        pub kind: Kind,
        /// Last text set/read. Kept in Rust because reading a `UILabel` back
        /// through `text` is cheap but the `UITextField` path needs the
        /// pre-edit value for the text-changed shim's dedup guard anyway.
        pub text: String,
        /// `set_hexpand` / `set_vexpand` request. UIKit has no expand flag;
        /// the box appender turns this into a flexible constraint or a
        /// `UIStackView` fill distribution, exactly like Android's
        /// LinearLayout weight (ANDROID_GUIDELINES.md §6).
        pub hexpand: bool,
        pub vexpand: bool,
        /// `set_width_chars(n)`: a minimum width in characters, so an empty
        /// `UITextField` (which measures ~0) stays tappable.
        pub min_chars: i32,
    ...
    /// True when `handle` is a canvas: canvases grow, chrome wraps (the iOS
    ...
    /// Class name the host must register for canvases (`SheetView`), set by
    /// the host cdylib before `init_with_root`. Mirrors Android's
    /// `set_sheet_view_class`; unset falls back to a plain `UIView`, which
    /// builds and lays out but never draws (the tree stays valid).
