...
impl Entry {
    ...
    pub fn get_text(&self) -> Option<String> { Some(self.0.text().to_string()) }
    /// Caret position as a **character** index (GTK counts chars, matching
    /// `crate::gui::gui_backend`'s `edit_caret`). `None` when the widget has
    /// no caret (out of range / no focus concept).
    pub fn get_position(&self) -> Option<usize> {
        let p = self.0.position();
        if p < 0 { None } else { Some(p as usize) }
    }
    /// Move the caret (character index); clamps to the text length.
    pub fn set_position(&self, pos: usize) {
        self.0.set_position(pos as i32);
    }
