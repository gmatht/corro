...
// ---------------------------------------------------------------------
// About / Full-help dialog text, shared by both UI backends.
//
// These were hoisted out of `ui::App` (the ratatui reference) so the
// pancurses backend can render the SAME dialog content without depending
// on the ratatui feature (pancurses-only builds drop `crate::ui`).
// `ui::App::about_page_body`/`help_page_body` delegate here.

    pub fn about_page_body() -> String {
        format!(
            "{name}\n\nVersion: {version}\n\n{about}\n\n{details}",
