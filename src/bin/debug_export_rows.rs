...
use corro::grid::{Grid, ColumnAddr, CellAddr, MARGIN_COLS};

fn main() {
    let mut g = Grid::new(1, 2);
    use corro::grid::HEADER_ROWS;
g.set(&CellAddr::Header { 
        row: (HEADER_ROWS - 1) as u32, 
        col: ColumnAddr::from_global(MARGIN_COLS + 1, 2)
    }, "=A*0.1 -- TAX".into());
g.set(&CellAddr::Header { 
        row: (HEADER_ROWS - 1) as u32, 
        col: ColumnAddr::from_global(MARGIN_COLS + 2, 2)
    }, "=TOTAL".into());
    ...
    g.set(&CellAddr::Footer { row: 0, col: ColumnAddr::Left(MARGIN_COLS - 1) }, "=TOTAL".into());
